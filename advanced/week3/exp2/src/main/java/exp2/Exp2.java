...
public class Exp2 {
    private static final String PATH1 = "./src/main/resources/list1.xml";
    private static final String PATH2 = "./src/main/resources/list2.xml";
    public static void writeXMLFileByDOM(List<Student> list) {
        try {
            ...
            TransformerFactory.newInstance().newTransformer().transform(new DOMSource(doc), new StreamResult(new File(PATH1)));
        }
        catch (Exception exc) {
            exc.printStackTrace();
        }
    }
    public static void readXMLFileByDOM() {
        try {
            Document doc = DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(PATH1);
    ...
    public static void writeXMLFileByStAX(List<Student> list) {
        try {
            XMLStreamWriter xmlStreamWriter = XMLOutputFactory.newInstance().createXMLStreamWriter(new FileWriter(PATH2));
    ...
    public static void readXMLFileByStAX() {
        try {
            XMLStreamReader xmlStreamReader = XMLInputFactory.newFactory().createXMLStreamReader(new FileReader(PATH2));
            while (xmlStreamReader.hasNext() == true) {
                int next = xmlStreamReader.next();
                if (next == XMLStreamConstants.START_ELEMENT) {
                    if (Objects.equals(String.valueOf(xmlStreamReader.getName()), "student")) {
                        System.out.print(xmlStreamReader.getAttributeValue(0) + " ");
                    }
                    else if (Objects.equals(String.valueOf(xmlStreamReader.getName()), "name")) {
                        System.out.print(xmlStreamReader.getElementText() + " ");
                    }
                    else if (Objects.equals(String.valueOf(xmlStreamReader.getName()), "gender")) {
                        System.out.print(xmlStreamReader.getElementText() + " ");
                    }
                    else if (Objects.equals(String.valueOf(xmlStreamReader.getName()), "birth")) {
                        System.out.print(xmlStreamReader.getElementText() + " ");
                    }
                    else if (Objects.equals(String.valueOf(xmlStreamReader.getName()), "major")) {
    ...
    public static void main(String[] args) {
        ...
        writeXMLFileByDOM(list);
        readXMLFileByDOM();
        writeXMLFileByStAX(list);
        readXMLFileByStAX();
    }
}