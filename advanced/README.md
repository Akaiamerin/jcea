...
使用 `Timer` 类、`ScheduleExecutorService` 类、`Quartz` 的 `CronTrigger` 触发分别实现每 `5` 秒向 `txt` 文档写入当前时间并在屏幕打印出来。
...
实现用户信息增删改查页面，数据库使用 `MyBatis` 和 `Druid` 技术。
...
前端使用 `Axios` 技术，数据库使用 `MyBatis` 和 `Druid` 技术。
