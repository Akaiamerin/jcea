...
用二维数组分别实现两个 $3 \times 3$ 的矩阵相加和相乘，并输出相应结果。
...
返回一个“分子/分母”的字符串形式，若分母为 `1`, 则返冋分子。
...
设计 `Eatable` 接口，该接口只有一个 `eat` 抽象方法。使用 `Lambda` 表达式创建 `3` 个 `Eatable` 接口对象，分别代表小狗、小猫、小鸡，三者执行各自的 `eat` 方法后会输出不同的文本。小狗爱吃骨头、小猫爱吃鱼、小鸡爱吃毛毛虫。
...
一个容量为 `100` 升的水池，在水池设置 `3` 个出水口，`A` 出水口每秒排出 `1` 升水，`B` 出水口一秒排出 `2` 升水，`C` 出口一秒排出 `3` 升水，每一次只能随机打开一个出口排水，使用线程模拟 `3` 个出水口随机排水的场景，并计算出多少秒后水池的水被排光。
...
使用 `Java` 程序在数据库创建一个 `emp` 表，表结构如下所示：
